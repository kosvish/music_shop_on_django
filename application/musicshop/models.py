...
class AlbumArtist(models.Model):
    name = models.CharField(max_length=255, verbose_name='Альбомы музыканта')
    slug = models.SlugField()
    image = models.ImageField(upload_to=upload_function, null=True, blank=True)
...
class Artist(models.Model):
    ...
    albums_of_artist = models.ManyToManyField(AlbumArtist, verbose_name='Альбомы артиста', related_name='albums_artist')
    image_gallery = GenericRelation('imagegallery')
    description_of_artist = models.TextField(verbose_name='Описание исполнителя', default='Описание появится позже')
    slug = models.SlugField()
    image = models.ImageField(upload_to=upload_function, null=True, blank=True)
...
class AlbumManager(models.Manager):
    ...
    def get_month_bestseller(self):
        ...
        # Блок кода в консоле SQL для вывода продаж месяца
        query = f"""
...
class CartProduct(models.Model):
    ...
    # Строчка кода для анонимных покупателей
...
# Метод для непрочитанных сообщений
...
# Функция которая проверяет send_notifications для избежания некорректности выполнения
...
# Функция сигнала, которая проверяет появлися ли товар на складе , если да , оповещает пользователей сайта
